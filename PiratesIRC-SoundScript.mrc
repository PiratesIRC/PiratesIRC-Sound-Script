...
;-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
alias PiratesIRC-Sound.version return 0.30.1
...
alias PiratesIRC-Sounds.PlaySound {
  ...
  if (%file) {
    ...
    if ($exists(%file)) {
      ...
      set %piratesirc-sounds.downloadattempts
    }
    else {
      inc %piratesirc-sounds.downloadattempts
      if (%piratesirc-sounds.downloadattempts < 3) {
      ...
      else PiratesIRC-Sounds.WriteLog Unable to download %file after %piratesirc-sounds.downloadattempts attempts!
    }
  }
}
